...
namespace MMBotGA.ga
{
    class SpreadChromosome : ChromosomeBase, ICustomChromosome
    {
        ...
        private readonly IStrategy _strategy; //Sem treba nacpat CustomStrategy

        protected SpreadChromosome(IStrategy fixedStrategy, bool finalize) : base(2)
        {
            _strategy = fixedStrategy;
            Factory = new GeneFactory(this);

            // max is exclusive
            Stdev = Factory.Create(() => RandomizationProvider.Current.GetDouble(1, 120));
            Sma = Factory.Create(() => RandomizationProvider.Current.GetDouble(1, 120));
        ...
        public SpreadChromosome(IStrategy fixedStrategy) : this(fixedStrategy, true) //Sem treba nacpat customStrategy
        { }
